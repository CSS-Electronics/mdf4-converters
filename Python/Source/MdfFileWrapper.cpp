...
MdfFileWrapper::MdfFileWrapper(Py::PythonClassInstance *self, Py::Tuple &args, Py::Dict &kwds) :
    ...
    try {
        if (args.size() > 1) {
            ...
            stream = std::make_shared<mdf::python::CallbackBuffer>(wrapper, cacheSize);
        } else {
...
Py::Object MdfFileWrapper::GetCANIterator() {
    ...
    mdf::RecordIterator<const mdf::CANRecord> iterator;
    ...
    return Py::asObject(new MdfCANIteratorWrapper(iterator));
}
...
static Py::Object createNumpyArray(std::size_t size, std::string const& dtype) {
    ...
    auto result = np.callMemberFunction("empty", Py::TupleN(Py::Long(size), dataType));
