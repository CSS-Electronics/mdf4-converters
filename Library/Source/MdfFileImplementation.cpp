...
namespace mdf {
  ...
  bool MdfFileImplementation::finalize_setLengthOfLastDTBlock() {
    ...
    if (highestDataBlock) {
      ...
      if (iter != std::end(blockStorage->getBlockMap().left)) {
        ...
        throw std::runtime_error("Data block is not bounded by end of file but by another block. Not supported in library.");
      } else {
        MdfHeader dtHeader = highestDataBlock->getHeader();
        dtHeader.blockSize = mmap.size() - highestDataBlockLocation;
    ...
    while (dgBlock) {
      ...
      for (auto const &block: cgBlocks) {
        ...
        if (nextCGBlock && nextCGBlock->isVLSD()) {
          ...
          std::shared_ptr<SDBlockDiscontinuous> sdBlock = std::make_shared<SDBlockDiscontinuous>(records);
          ...
          recordsIDs.erase(std::remove(std::begin(recordsIDs), std::end(recordsIDs), nextCGBlock->getRecordID()),
            std::end(recordsIDs));
  ...
  bool MdfFileImplementation::sort_CGtoDG() {
    ...
    while (dgBlock) {
      ...
      if (followingCGBlock) {
        ...
        if (recordsIDs.size() > 1) {
          replacementDTBlockForNewDGBlock = std::make_shared<DTBlockMultipleRecordIDs>(*currentDataBlock, recordsIDs);
        } else if (recordsIDs.size() == 1) {
          replacementDTBlockForNewDGBlock = std::make_shared<DTBlockSingleDiscontinuous>(*currentDataBlock, recordsIDs[0]);
        }
  ...
  RecordIterator<CANRecord const> MdfFileImplementation::getCANIterator() {
    ...
    if (dgBlock) {
      ...
      std::unique_ptr<IIterator<CANRecord const>> iter = std::make_unique<GenericIterator<CANRecord>>(dgBlock, fileInfo.Time);
  ...
  RecordIterator<LINRecord const> MdfFileImplementation::getLINIterator() {
    ...
    if (dgBlock) {
      // Create a new iterator from this.
      std::unique_ptr<IIterator<LINRecord const>> iter = std::make_unique<GenericIterator<LINRecord>>(dgBlock, fileInfo.Time);
