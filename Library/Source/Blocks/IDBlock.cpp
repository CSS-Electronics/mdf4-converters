...
namespace mdf {
    ...
    bool IDBlock::save(std::streambuf *stream) {
        IDBlockData data;
        ...
        if((finalizationFlags == FinalizationFlags_None) && (customFinalizationFlags == 0)) {
            // Write as finalized.
            std::copy(std::cbegin(finalizedIdentifierStr), std::cend(finalizedIdentifierStr), data.identifierStr);
        } else {
            // Write as unfinalized.
            std::copy(std::cbegin(unfinalizedIdentifierStr), std::cend(unfinalizedIdentifierStr), data.identifierStr);
            data.finalizationFlags = finalizationFlags;
            data.customFinalizationFlags = customFinalizationFlags;
        }

        // Write tool and version information.
        data.versionNum = versionNum;
        std::copy(std::cbegin(versionStr), std::cend(versionStr), data.versionStr);
        std::copy(std::cbegin(creationTool), std::cend(creationTool), data.toolStr);
