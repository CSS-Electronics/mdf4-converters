...
namespace mdf {
    ...
    std::shared_ptr<TXBlock> CGBlock::getTXBlock() const {
        return std::dynamic_pointer_cast<TXBlock>(links[2]);
    }
    ...
    bool CGBlock::saveBlockData(std::streambuf *stream) {
        CGBlockData data;
        ...
        data.flags = flags;
        data.path_separator = pathSeparator;
        data.data_bytes = dataBytes;
        data.inval_bytes = invalDataBytes;
