...
namespace mdf {
  ...
  GenericIterator<RecordType>::GenericIterator(std::shared_ptr<DGBlock> dgBlock, std::chrono::nanoseconds offset, std::size_t start) : start(start),
                                                                                                      current(0) {
    ...
    for (auto const &cnBlock: cnBlocks) {
      ...
      boost::function<uint8_t const *(uint64_t)> f;
    ...
    std::function<uint8_t const *(std::size_t)> ff = functorStorage[0];
  ...
  void GenericIterator<RecordType>::mapDataToRecord() {
    ...
    uint8_t const *dataPtr = func(current);
    ...
    bitData.clear();
    bitData.init_from_block_range(dataPtr, dataPtr + bitData.capacity() / 8);
    ...
    auto f = [&](auto index) {
      ...
      if (iter != mapping.end()) {
        ...
        if (!entry.dataFunc) {
          ...
          uint8_t const *dataPtr = entry.dataFunc(rawValue);
          assign2(boost::fusion::at_c<index>(record), dataPtr);
        }
